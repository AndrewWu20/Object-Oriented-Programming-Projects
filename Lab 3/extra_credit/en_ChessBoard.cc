...
bool ChessBoard::movePiece(int fromRow, int fromColumn, int toRow, int toColumn){
    std::cout<<"Moving ("<<fromRow<<", "<<fromColumn<<") to ("<<toRow<<", "<<toColumn<<")\n";
    ...
    if(movingPiece != nullptr){ //a piece is there to move
        ...
        if(movingPiece->getColor() == turn){
            ...
            if(isValidMove(fromRow, fromColumn, toRow, toColumn) == true){
                ...
                else if((movingPiece->getType() == Pawn) && ((toColumn == (fromColumn-1))||(toColumn == (fromColumn+1)))){ //en passant black taking
                    std::cout<<"here\n";
                    delete board.at(fromRow).at(toColumn);
                    board.at(fromRow).at(toColumn) = nullptr;
                    
                }
                // else if((movingPiece->getType() == Pawn) && (movingPiece->getColor() == White) && ((toColumn == fromColumn-1)||(toColumn = fromColumn+1))){ //en passant white taking
                //     delete board.at(fromRow).at(toColumn);
                //     board.at(fromRow).at(toColumn) = nullptr;
                // }
