    ...
    if(movingPiece != nullptr){ //a piece is there to move
        ...
        if(movingPiece->getColor() == turn){
            ...
            if(isValidMove(fromRow, fromColumn, toRow, toColumn) == true){
                ...
                if((movingPiece->getType() == King) && (fromColumn+2 == toColumn)) { //castling right
                    for(int c = toColumn+1; c < numCols; c++){
                        Student::ChessPiece *tempPiece = getPiece(toRow, c);
                        if(tempPiece != nullptr && tempPiece->getColor() == pieceColor && tempPiece->getType() == Rook){
                            board.at(toRow).at(toColumn-1) = tempPiece;
                            tempPiece->setPosition(toRow, toColumn-1);
                            board.at(fromRow).at(c) = nullptr;
                        }
                    }
                } else if((movingPiece->getType() == King) && (fromColumn-2 == toColumn)){ //castling left
                    for(int c = toColumn-1; c >= 0; c--){
                        Student::ChessPiece *tempPiece = getPiece(toRow, c);
                        if(tempPiece != nullptr && tempPiece->getColor() == pieceColor && tempPiece->getType() == Rook){
                            board.at(toRow).at(toColumn+1) = tempPiece;
                            tempPiece->setPosition(toRow, toColumn+1);
...
bool ChessBoard::isValidMove(int fromRow, int fromColumn, int toRow, int toColumn){
    ...
    if(testPiece != nullptr){ //Piece exists in location
        ...
        for(int r = 0; r < numRows; r++){ //check if king is under threat
            for(int c = 0; c < numCols; c++){
                if((board.at(r).at(c) != nullptr)){
                    Student::ChessPiece *tempPiece = getPiece(r, c);
                    if((tempPiece->getColor() == pieceColor) && (tempPiece->getType() == King)){
        ...
        if((testPiece->getType() == King) && (fromColumn+2 == toColumn)) { //castling right
            Student::ChessBoard tempBoard = *this;
            tempBoard.tempMove(fromRow, fromColumn, toRow, toColumn-1);
            if(tempBoard.isPieceUnderThreat(toRow, toColumn-1) == true){
                return false;
            }
        } else if((testPiece->getType() == King) && (fromColumn-2 == toColumn)){ //castling left
            Student::ChessBoard tempBoard = *this;
            tempBoard.tempMove(fromRow, fromColumn, toRow, toColumn+1);
            if(tempBoard.isPieceUnderThreat(toRow, toColumn+1) == true){
                return false;
