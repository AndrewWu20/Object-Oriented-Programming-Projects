#include "../Chess.h"
#include "../ChessBoard.hh"
#include "../ChessPiece.hh"
#include "../PawnPiece.hh"
#include "../RookPiece.hh"
#include "../BishopPiece.hh"
#include "../KingPiece.hh"
#include <iostream>

void test_part3_8x8_9(){
	Student::ChessBoard sBoard(8, 8);
	sBoard.createChessPiece(White, Bishop, 7, 0);
	sBoard.createChessPiece(Black, Rook, 5, 2);
	sBoard.createChessPiece(White, Rook, 3, 0);
	sBoard.createChessPiece(Black, Rook, 1, 4);
	sBoard.createChessPiece(White, Rook, 3, 7);
	sBoard.createChessPiece(Black, Bishop, 1, 1);
	sBoard.createChessPiece(White, Rook, 5, 1);
	sBoard.createChessPiece(White, Rook, 0, 2);
	sBoard.createChessPiece(White, Pawn, 2, 6);
	sBoard.createChessPiece(White, Bishop, 1, 1);
	sBoard.createChessPiece(White, Pawn, 1, 6);
	sBoard.createChessPiece(Black, Bishop, 3, 5);
	sBoard.createChessPiece(Black, Pawn, 5, 4);
	sBoard.createChessPiece(Black, Bishop, 5, 2);
	sBoard.createChessPiece(White, Pawn, 3, 3);
	sBoard.createChessPiece(White, Rook, 5, 0);
	sBoard.createChessPiece(Black, King, 7, 4);
	sBoard.createChessPiece(White, King, 6, 0);
	sBoard.movePiece(1,1,2,0);
	sBoard.movePiece(3,5,6,2);
	sBoard.movePiece(2,0,1,1);
	sBoard.movePiece(1,2,3,0);
	sBoard.movePiece(7,4,6,3);
	sBoard.movePiece(3,0,4,0);
	sBoard.movePiece(1,4,1,2);
	sBoard.movePiece(0,0,1,7);
	sBoard.movePiece(0,2,0,7);
	sBoard.movePiece(6,2,5,3);
	sBoard.movePiece(0,7,0,6);
	sBoard.movePiece(5,5,4,4);
	sBoard.movePiece(5,3,6,4);
	sBoard.movePiece(7,0,5,2);
	sBoard.movePiece(6,3,7,3);
	sBoard.movePiece(1,3,0,3);
	sBoard.movePiece(4,0,1,0);
	sBoard.movePiece(7,3,7,2);
	sBoard.movePiece(3,7,3,4);
	sBoard.movePiece(1,5,2,5);
	sBoard.movePiece(1,2,0,2);
	sBoard.movePiece(3,4,3,6);
	sBoard.movePiece(6,4,3,7);
	sBoard.movePiece(6,2,6,7);
	sBoard.movePiece(5,1,6,1);
	sBoard.movePiece(5,4,6,4);
	sBoard.movePiece(5,2,2,5);
	sBoard.movePiece(3,5,1,4);
	sBoard.movePiece(0,2,6,2);
	sBoard.movePiece(5,0,5,2);
	sBoard.movePiece(6,4,7,4);
	sBoard.movePiece(7,6,1,0);
	sBoard.movePiece(0,6,0,1);
	sBoard.movePiece(3,7,2,6);
	sBoard.movePiece(5,2,3,2);
	sBoard.movePiece(1,6,5,3);
	sBoard.movePiece(2,6,1,5);
	sBoard.movePiece(6,1,6,2);
	sBoard.movePiece(7,2,7,3);
	sBoard.movePiece(4,7,6,7);
	sBoard.movePiece(6,2,7,2);
	sBoard.movePiece(7,3,6,3);
	sBoard.movePiece(6,0,7,0);
	sBoard.movePiece(1,7,5,0);
	sBoard.movePiece(6,3,6,4);
	sBoard.movePiece(7,2,7,1);
	sBoard.movePiece(1,5,3,7);
	sBoard.movePiece(6,0,3,3);
	sBoard.movePiece(1,0,3,0);
	sBoard.movePiece(3,7,5,5);
	sBoard.movePiece(3,6,3,4);
	sBoard.movePiece(5,5,1,7);
	sBoard.movePiece(6,4,6,5);
	sBoard.movePiece(1,1,0,2);
	sBoard.movePiece(5,5,4,4);
	sBoard.movePiece(6,6,1,7);
	sBoard.movePiece(7,1,6,1);
	sBoard.movePiece(6,5,5,5);
	sBoard.movePiece(0,1,3,1);
	sBoard.movePiece(5,3,5,2);
	sBoard.movePiece(4,4,2,6);
	sBoard.movePiece(3,3,2,3);
	sBoard.movePiece(2,6,0,4);
	sBoard.movePiece(2,0,4,1);

	std::cout << sBoard.displayBoard().str() << "\n";
	return;
}
